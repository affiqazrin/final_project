...
class StatementProcessor:
    ...
    def read_insx_config(self, f, insg_cycle):
        ...
        sql=f"SELECT * FROM insg_statement_config WHERE insg_calculation_file_name='{insx_file_name}' AND insg_cycle='{insg_cycle}'"
    ...
    def process_data(self, f, df, insg_to_insx, sheet, cycle, month):
        ...
        df_filtered['insx_month']=self.params['insx_month']
        df_filtered['insx_cycle']=self.params['insx_cycle']
        df_filtered['insx_version']=self.params['insx_version']
        df_filtered['insx_src_file']=insg_to_insx['insg_src_file']            
                   
        return df_filtered
    
    def process_statement(self, f, sheet, insg_cycle, cycle, month):        
        df, lst_sheet, f=self.read_insx_file(self, f)
        
        insx_file_name=os.path.basename(f).split[0]
        print(f'Execute statement: {insx_file_name}')
        
        insg_to_insx=read_insx_config(self, f, insg_cycle)
        processed_df=process_data(self, df, insg_to_insx, insx_file_name, sheet, cycle, month)
